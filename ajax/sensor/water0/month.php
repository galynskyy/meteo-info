<?php
	/* SQL */
		...
		/* /Connect */
		/* Query */
			$query = $connection->prepare("SELECT temperature, humidity, date FROM weather WHERE (date >= DATE_SUB(CURRENT_DATE, INTERVAL 1 MONTH)) AND (TIME(date) BETWEEN :time_start AND :time_end)");
